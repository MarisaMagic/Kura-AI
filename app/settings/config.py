...
class Settings(BaseSettings):
    ...
    # ---- 会话存储瘦身与配额（防滥用 / 防库膨胀）----
    # 消息 content 预览列字符上限（完整正文存 content_json，预览仅用于列表/展示）
    CHAT_MESSAGE_PREVIEW_CHARS: int = 2000
    # 单条助手消息附加 JSON（rag_trace/rag_steps/thinking_items/sources）字符上限；超出丢调试字段
    CHAT_EXTRA_JSON_MAX_CHARS: int = 8000
    # 附加 JSON 列表长度上限
    CHAT_RAG_STEPS_MAX: int = 50
    CHAT_THINKING_ITEMS_MAX: int = 200
    CHAT_SOURCES_MAX: int = 50
    # 同一轮重新生成最多保留的助手版本数，超出淘汰最旧（0=不限）
    CHAT_MAX_ASSISTANT_VERSIONS: int = 5
    # 每用户每智能体会话数上限 / 每会话消息行上限（0=不限）
    CHAT_MAX_SESSIONS_PER_USER_AGENT: int = 200
    CHAT_MAX_MESSAGES_PER_SESSION: int = 4000
    # 闲置会话留存天数（0=关闭，需用 scripts/purge_idle_sessions.py 清理）
    CHAT_SESSION_RETENTION_DAYS: int = 0
    CHAT_SESSION_RETENTION_BATCH: int = 200
    ...
    # 压缩在请求内同步执行，该值越小单次摘要越快（用户可感知的停顿越短）
    CHAT_COMPACT_SUMMARIZER_INPUT_TOKENS: int = 24000
    ...
    # 估算用量超过 effective * ratio 就开始清理（未达阈值时零改动）；
    # 该闸门同时用于「压缩前预处理」：先清理再判断是否真的需要付 LLM 摘要
    CHAT_MICROCOMPACT_TRIGGER_RATIO: float = 0.6
    # 单个工具结果超过该 token 数时，无论新旧都先截断（保留头尾）
    CHAT_MICROCOMPACT_MAX_RESULT_TOKENS: int = 6000

    # ---- 会话原文翻牌工具 read_session_history（精确取证通道）----
    # 同一用户提问轮次内最多成功调用次数，防止模型反复翻库
    CHAT_HISTORY_TOOL_MAX_CALLS: int = 2

    # ---- 跨会话用户长期记忆（PostgreSQL 存储，无向量库/embedding）----
    # 模型通过 read/save/forget_user_memory 工具按需读写，不在压缩摘要阶段自动抽取
    CHAT_USER_MEMORY_ENABLED: bool = True
    CHAT_MEMORY_FACT_MAX_CHARS: int = 600
    # 每用户每智能体的长期记忆条数上限，超出按 updated_at 删最旧
    CHAT_MEMORY_USER_FACT_MAX: int = 500
    # 同一轮内 save_user_memory 最多成功写入条数
    CHAT_MEMORY_WRITE_MAX_PER_TURN: int = 3
    # read_user_memory 一次最多返回的条数 / 工具返回 token 预算
    CHAT_MEMORY_READ_MAX_ITEMS: int = 50
    ...
    KB_STRUCTURAL_CHUNKING_ENABLED: bool = True
    # 源码 AST 切分开关（tree-sitter 不可用时自动降级，不受此开关影响）
    KB_CODE_CHUNKING_ENABLED: bool = True
    # 原子块阈值：字符数与 UTF-8 字节数同时满足才整块保留
    # 注意 Milvus text 字段 max_length=2000 且按字节计，多字节中文须留足余量
    KB_ATOMIC_BLOCK_MAX_CHARS: int = 1200
    KB_ATOMIC_BLOCK_MAX_BYTES: int = 1800
    # 表格每块最多数据行数（与字节阈值共同决定切分粒度）
    KB_TABLE_MAX_ROWS_PER_CHUNK: int = 200
    # PDF 表格提取（PyMuPDF find_tables）；最小行列数用于抑制多栏正文误判
    KB_PDF_TABLE_EXTRACTION: bool = True
    KB_PDF_TABLE_MIN_ROWS: int = 2
    KB_PDF_TABLE_MIN_COLS: int = 2
    # 单文档叶子块上限：超出直接判失败，避免超大文件打爆嵌入成本
    KB_MAX_CHUNKS_PER_DOC: int = 20000
    # 分块管线版本：与 KbDocument.chunk_pipeline_version 不一致时强制重建（不再按 content_hash 跳过）
    KB_CHUNK_PIPELINE_VERSION: int = 2
    # code_language 字段长度上限（Milvus VARCHAR(40)）
    KB_CODE_LANGUAGE_MAX_CHARS: int = 40
